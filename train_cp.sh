...
#SBATCH --gres=gpu:2                                           # generic resource required (here requires 1 GPUs)
#SBATCH --mem=96GB                                              # memory pool for all cores (here set to 16 GB)

# Configure notifications 
#SBATCH --mail-type=END                                         # Type of email notifications will be sent (here set to END, which means an email will be sent when the job is done)
#SBATCH --mail-type=FAIL                                        # Type of email notifications will be sent (here set to FAIL, which means an email will be sent when the job is fail to complete)
#SBATCH --mail-user=a1675776@adelaide.edu.au                    # Email to which notification will be sent
...
python tools/train_net.py --num-gpus 2 --config-file configs/FCOS-COCO-Detection/fcos_R_18_1x-FPN_BN-dorefa.yaml