...
def forward_hook(module, input, output):
    ...
    print(name)
    input_maps[name] = input[0]
    feature_maps[name] = output
    if hasattr(module, 'weight_clip_value'):
        weight_clip_value[name] = module.weight_clip_value
        activation_clip_value[name] = module.activation_clip_value
    
    if name == 'proposal_generator.fcos_head.bbox_pred':
        i = i + 1
        save_dict = {
        ...
        if i == 5:
            torch.save(save_dict, os.path.join(save_path, 'save.pth'))
            assert False
...
def main(args):
    ...
    load_bn(cfg.MODEL.WEIGHTS, model)

    save_path = cfg.OUTPUT_DIR

    for name, module in model.named_modules():
        # if isinstance(module, torch.nn.Conv2d):
        if isinstance(module, (torch.nn.BatchNorm2d, torch.nn.Conv2d)):
            # print(name)
            module.name = name
