...
from detectron2_ofa.layers import cat
...
class LightROIHeads(ROIHeads):
    def __init__(self, cfg, input_shape):
        super(LightROIHeads, self).__init__(cfg, input_shape)
        # fmt: off
        pooler_resolution   = cfg.MODEL.LIGHT_HEAD.POOLER_RESOLUTION
        pooler_scales       = tuple(1.0 / self.feature_strides[k] for k in self.in_features)
        sampling_ratio      = cfg.MODEL.LIGHT_HEAD.POOLER_SAMPLING_RATIO
        pooler_type         = cfg.MODEL.LIGHT_HEAD.POOLER_TYPE
        self.mask_size      = cfg.MODEL.LIGHT_HEAD.MASK_SIZE
        canonical_size_box  = cfg.MODEL.LIGHT_HEAD.CANONICAL_SIZE_BOX
        canonical_size_mask = cfg.MODEL.LIGHT_HEAD.CANONICAL_SIZE_MASK
        canonical_level     = cfg.MODEL.LIGHT_HEAD.CANONICAL_LEVEL
        # fmt: on
