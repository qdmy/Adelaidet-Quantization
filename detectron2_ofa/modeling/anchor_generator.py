...
from detectron2_ofa.structures import Boxes, RotatedBoxes
...
class DefaultAnchorGenerator(nn.Module):
    ...
    def _calculate_anchors(self, sizes, aspect_ratios):
        ...
        if len(sizes) == 1:
            sizes *= self.num_features # broadcast
    ...
    def generate_cell_anchors(self, sizes=(32, 64, 128, 256, 512), aspect_ratios=(0.5, 1, 2)):
        ...
        for size in sizes:
            ...
            for aspect_ratio in aspect_ratios:
                ...
                # w = sqrt(s * s / a)
                # h = a * w
                w = math.sqrt(area / aspect_ratio) # 得到scale后的面积，再算width
                h = aspect_ratio * w
                x0, y0, x1, y1 = -w / 2.0, -h / 2.0, w / 2.0, h / 2.0 # 每个anchor的中心点坐标为(0,0)
    ...
    def forward(self, features):
        ...
        grid_sizes = [feature_map.shape[-2:] for feature_map in features] # get each resolution
        anchors_over_all_feature_maps = self.grid_anchors(grid_sizes) # 按照指定stride移动后的所有anchor可能位置
