...
class FPN(Backbone):
    ...
    def __init__(
        ...
        self.lateral_convs = lateral_convs[::-1] # [::-1]是倒序
    ...
    def forward(self, x):
        ...
        results = []
        prev_features = self.lateral_convs[0](x[0]) # 把bottom up输出的最小的feature处理一下作为FPN的起点
        ...
        for features, lateral_conv, output_conv in zip(
            ...
            if self.use_relu:
                ...
                results.insert(0, F.relu_(output_conv(prev_features))) # 插入到0号位置
            else:
...
class LastLevelP6P7(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, in_features="res5", norm="", activation=False):
        ...
        self.use_relu = activation
        use_bias = norm == ""
        self.p6 = Conv2d(in_channels, out_channels, kernel_size=3, stride=2, padding=1, bias=use_bias, norm=get_norm(norm, out_channels))
        self.p7 = Conv2d(out_channels, out_channels, kernel_size=3, stride=2, padding=1, bias=use_bias, norm=get_norm(norm, out_channels))
...
@BACKBONE_REGISTRY.register()
def build_fcos_qresnet_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
        cfg: a detectron2 CfgNode
...
@BACKBONE_REGISTRY.register()
def build_fcos_resnet_lpf_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
        cfg: a detectron2 CfgNode
