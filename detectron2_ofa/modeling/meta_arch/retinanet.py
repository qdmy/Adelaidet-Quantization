...
class RetinaNet(nn.Module):
    ...
    super_targets_masks, super_targets_inverse_masks):
        ...
        Args:
            ...
            ofa for loss: super_targets_mask, super_targets_inverse_mask
        Returns:
            dict[str: Tensor]:
    ...
    def inference(self, box_cls, box_delta, anchors, images):
        ...
        ofa for inference: super_targets_idx, super_target
...
class RetinaNetHead(nn.Module):
    ...
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        self.cls_score = nn.Conv2d( # 用卷积替代FC
    ...
    def update_norm_layer(self, index=0):
        if self.norm in ['BN', 'SyncBN']:
            ...
            for head in heads:
                for i in range(self.num_convs):
                    getattr(self, "{}_subnet".format(head))[1 + 3*i] = getattr(self, "{}_norm{}".format(head, i))[index] # 1+3i的位置正是对应head里每个BN的位置
        pass
    ...
    def forward(self, features):
        ...
        for l, feature in enumerate(features):
            self.update_norm_layer(l) # 这里为什么要更新norm层？是因为模型在初始化时给每个resolution的feature定义了对应的BN，这是就要指明对应的head里是哪一个BN
