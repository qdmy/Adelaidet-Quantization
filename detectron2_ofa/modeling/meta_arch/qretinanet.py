...
from detectron2_ofa.layers import ShapeSpec, batched_nms, cat, get_norm
from detectron2_ofa.structures import Boxes, ImageList, Instances, pairwise_iou
from detectron2_ofa.utils.logger import log_first_n
...
class QRetinaNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.head = QRetinaNetHead(cfg, feature_shapes)
...
class QRetinaNetNotSharedHead(nn.Module):
    """
    ...
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        for i in range(self.num_convs):
            ...
            if self.norm != "":
                cls_subnet.append(get_norm(self.norm, in_channels))
            ...
            if self.norm in ['BN', 'SyncBN', 'GN'] and not self.shared_norm:
                ...
                cls_subnet[-2] = getattr(self, "{}_norm{}".format("cls", i))[0]
            ...
            if self.norm != "":
                bbox_subnet.append(get_norm(self.norm, in_channels))
            bbox_subnet.append(nn.ReLU(inplace=True))
            if self.norm in ['BN', 'SyncBN', 'GN'] and not self.shared_norm:
                self.add_module('{}_norm{}'.format("bbox", i),
                    nn.ModuleList([get_norm(self.norm, in_channels) for j in range(self.feature_num)]))
                bbox_subnet[-2] = getattr(self, "{}_norm{}".format("bbox", i))[0]
        ...
        if self.quantize_first_and_last:
            ...
            self.bbox_pred = conv(in_channels, num_anchors * 4, kernel_size=3, stride=1, padding=1,
                bits_weights=self.bits_weights,
                bits_activations=self.bits_activations,
                merge_bn=self.merge_bn)
        else:
    ...
    def update_norm_layer(self, index=0):
        if self.norm in ['BN', 'SyncBN', 'GN'] and not self.shared_norm:
            ...
            for head in heads:
                for i in range(self.num_convs):
                    getattr(self, "{}_subnet".format(head))[1 + 3*i] = getattr(self, "{}_norm{}".format(head, i))[index]
        pass
...
class QRetinaNetHead(nn.Module):
    """
    The head used in RetinaNet for object classification and box regression.
    It has two subnets for the two tasks, with a common structure but separate parameters.
    """
    ...
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        self.num_convs        = cfg.MODEL.RETINANET.NUM_CONVS
        self.shared_norm      = cfg.MODEL.RETINANET.SHARED_NORM

        self.bits_weights         = cfg.MODEL.QUANTIZATION.wt_bit
        self.bits_activations     = cfg.MODEL.QUANTIZATION.fm_bit
        self.first_last_bits_weights = cfg.MODEL.QUANTIZATION.first_last_wt_bit
        self.first_last_bits_activations = cfg.MODEL.QUANTIZATION.first_last_fm_bit
        self.quan_type            = cfg.MODEL.QUANTIZATION.QUAN_TYPE
        self.quantize_first_and_last = cfg.MODEL.QUANTIZATION.QUANTIZE_FIRST_AND_LAST
        self.merge_bn = cfg.MODEL.QUANTIZATION.MERGE_BN
        ...
        if self.quantize_first_and_last:
            self.cls_score = conv(
                in_channels, num_anchors * num_classes, kernel_size=3, stride=1, padding=1,
                bits_weights=self.first_last_bits_weights,
                bits_activations=self.first_last_bits_activations,
                merge_bn=self.merge_bn
            )
            self.bbox_pred = conv(in_channels, num_anchors * 4, kernel_size=3, stride=1, padding=1,
                bits_weights=self.first_last_bits_weights,
