...
from detectron2_ofa.structures import BoxMode
from detectron2_ofa.utils.comm import get_world_size
...
def build_detection_train_loader(cfg, mapper=None):
    ...
    group_bin_edges = [1] if cfg.DATALOADER.ASPECT_RATIO_GROUPING else [] # 如果为 True，则每个批次应仅包含纵横比兼容的图像。这会将人像图像组合在一起，并且风景图像不会与人像图像一起批处理。
    aspect_ratios = [float(img[0]["height"]) / float(img[0]["width"]) for img in dataset]
