...
from detectron2_ofa.structures import BitMasks, Boxes, BoxMode, Keypoints, PolygonMasks, RotatedBoxes
...
class Visualizer:
    ...
    def vis_inst(self, inst):
        inst = inst.squeeze().cpu().numpy()
        fig, axes = plt.subplots(nrows=1, ncols=3)
        for i, field_map in enumerate(inst):
            axes[i].imshow(field_map)
        plt.show()

    def draw_sem_seg(self, sem_seg, area_threshold=None, alpha=0.8, thing_classes=False):
        """
